    ...
    <div class="cal-body">
      ...
      <div class="dates" >
        <div v-for="date in dayList" class="item"
         ...
         <!-- ****修改日历状态**** -->
        <span v-if="date.status ? (date.title =='任务已完成') : false" class="is-event"
            :style="{borderColor: customColor, backgroundColor: (date.date == selectedDay) ? customColor : 'inherit'}"></span>
        
          <span v-if="date.status ? (date.title == '任务截止') : false" class="is-event is-over"
             :style="{borderColor: customColor, backgroundColor: (date.date == selectedDay) ? customColor : 'inherit'}"></span> 
            <span v-if="date.status ? (date.title == '存在日程') : false" class="is-event is-existence"
             :style="{borderColor: customColor, backgroundColor: (date.date == selectedDay) ? customColor : 'inherit'}"></span> 
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    dayList () {
      ...
      // console.log(this.calendar.params.curYear);   //现在所在的日历年份
      // console.log(this.calendar.params.curMonth);  //现在所在得月份
      // console.log(this.calendar.params.getDat);   //现在所在得日期
